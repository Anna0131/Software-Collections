...
const login_page_bottom_refs = '<div style="margin-left:85%;"><a href="/tutorial">系統文件</a><br/><a href="mailto:tommy50508@gmail.com,s109241047@mail1.ncnu.edu.tw" style="float:left;">管理員</a></div>';
...
router.get('/sso', async function(req, res) {
	try {
    	...
    	res.send(`${login_page_html}<div><label>請使用 <a href='https://sso.ncnu.edu.tw'>NCNU SSO</a> 帳號登入</label><br/>帳號：<input id='account'/><br/><br/>密碼：<input type='password' id='password'/><br/><br/>驗證碼：<input id='captcha'/><br/></div><br/><img style='width:10%' src="data:image/png;base64,${screenshot}"/><br/><div style='display:none'><div id='csrf'>${csrf}</div><div id='xsrf'>${xsrf}</div><div id = 'session'>${session}</div></div><button onclick='submitLogin()'>送出</button><br/><br/>${login_page_bottom_refs}<script src = "/js/sso_login.js"></script><script src = 'https://cdnjs.cloudflare.com/ajax/libs/axios/0.27.2/axios.min.js'></script>`); // 回傳圖片
	}
	catch(e) {
		console.error(e);
